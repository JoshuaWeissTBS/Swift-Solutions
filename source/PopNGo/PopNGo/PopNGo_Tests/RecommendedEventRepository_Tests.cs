...
public class RecommendedEventRepositoryTests
{
    ...
    public void SetRecommendedEvents_ShouldSetEvents()
    {
        ...
        var userId2 = 2;
        var eventIds2 = new List<int> { 4, 5, 6 };
        ...
        // Assert
        var result_user_1 = _context.RecommendedEvents.Where(e => e.UserId == userId1).Select(e => e.EventId).ToList().OrderBy(e => e).ToList();
        var result_user_2 = _context.RecommendedEvents.Where(e => e.UserId == userId2).Select(e => e.EventId).ToList().OrderBy(e => e).ToList();

        Assert.That(result_user_1.Count, Is.EqualTo(3));
        Assert.That(result_user_1[0], Is.EqualTo(1));
        Assert.That(result_user_1[1], Is.EqualTo(2));
        Assert.That(result_user_1[2], Is.EqualTo(3));

        Assert.That(result_user_2.Count, Is.EqualTo(3));
        Assert.That(result_user_2[0], Is.EqualTo(4));
        Assert.That(result_user_2[1], Is.EqualTo(5));
        Assert.That(result_user_2[2], Is.EqualTo(6));
    }
}
